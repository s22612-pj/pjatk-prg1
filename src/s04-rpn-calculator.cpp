...
auto evaluate_divide_integers(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for //"};
    }
...
auto evaluate_divide_modulo(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for %"};
    }
...
auto evaluation_absolute_value(std::stack<double>& stack) -> void
{
    if (stack.size() < 1) {
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_divide_integers(stack);
            } else if (each == "%") {
                evaluate_divide_modulo(stack);
            } else if (each == "**") {
                evaluate_exponentiation(stack);
            } else if (each == "sqrt") {
                evaluate_square_root(stack);
            } else if (each == "abs") {
                evaluation_absolute_value(stack);
            } else {
